...
export const getScrollLeft = (element: HTMLElement) =>
  getNormalizedScrollLeft(element as HTMLElement, contentDirection || getContentDirection());

export const setScrollLeft = (element: HTMLElement, value: number) => {
...
/** @public */
const computeScrollIntoView = (target: Element, options: Options): ScrollAction[] => {
  ...
  for (let index = 0; index < frames.length; index++) {
    ...
    if (scrollingElement === frame) {
      ...
      const scrollLeft = getScrollLeft(frame as HTMLElement);
